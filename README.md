...
DeepLabV3(backbone: ResNet50) によるピクセル単位 multi-label セマンティックセグメンテーションのデモ実装である。
研究室で取り組んでいる **プラズマ波動のスペクトログラム上での自動検出パイプライン** から、
データポリシー上公開できない部分を全て差し替え、技術的本質のみを残したものである。
...
で実データおよびその派生物の再配布が禁止されているためであり、またデータセット作成者(研究室既卒学生)のアノテーション知見を
無断公開すべきでないという研究倫理上の判断による。

## なぜ MNIST で本物のタスク(プラズマ波動検出)と同等のスキルが示せるのか
...
<img width="1866" height="628" alt="Image" src="https://github.com/user-attachments/assets/70f7078e-15d0-46af-800c-23b877904d6e" />|
-|
学習データサンプル
- 0-9の数字をプラズマ波動に見立てて、画像内に映った数字を検出するために、分類タスクを行う。
- 画像内に散在する直線・曲線は、スペクトログラム上に乗るノイズを模している。
#### 数字検出分類タスクを行うにあたり、DeepLabV3(Segmentationタスクモデル)を採用している理由
- 専用の組み込みプロセッサで実装可能なモデル
- かつ、Segmentationモデルは空間情報を分類モデルより直接的に学習できる点で精度面で有利

## 結果
> 100 epoch


<img width="1200" height="400" alt="Image" src="https://github.com/user-attachments/assets/d62b9d04-21ea-455f-a82a-a6e7be9249d8" />|
-|
学習曲線|

<img width="1844" height="1268" alt="Image" src="https://github.com/user-attachments/assets/43b4782f-94f0-48d3-ad2c-13e200aad4ca" />|
-|
入力画像と各クラスの正解マスク(上)と、推論結果(下)

<img width="1189" height="398" alt="Image" src="https://github.com/user-attachments/assets/1cd429d0-5008-4511-96df-bf46a3935627" />|
-|
後処理により、学習タスクを分類問題としたときの混同行列
- Segmentation結果の特定の数字ラベルにおいて、閾値以上のピクセルが検出された場合に`True 1`, そうでない場合は`True 0`とする

Digit         |F1   |Precision    |Recall
-|-|-|-
digit_0|0.947|  0.899|1.0004
digit_1|0.934|  0.888|0.985
digit_2|0.911|  0.837|1.0003
digit_3|0.881|  0.787|1.000
digit_4|0.947|  0.913|0.9848
digit_5|0.907|  0.842|0.982
digit_6|0.934|  0.876|1.0000
digit_7|0.903|  0.823|1.0001
digit_8|0.887|  0.797|1.000
digit_9|0.880|  0.795|0.985
...
変更点は.ipynbファイル冒頭のマークダウンセルに表形式で記載している。
...
- 本リポジトリの主目的はデモであり、ハイパーパラメータ最適化は行っていない。
